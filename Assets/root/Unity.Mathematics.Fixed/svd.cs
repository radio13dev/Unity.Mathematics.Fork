...
namespace Unity.Mathematics.Fixed
{
    ...
    static public class svd
    {
        ...
        static void condSwap(bool c, ref fp x, ref fp y)
        {
            var tmp = x;
            x = math.select(x, y, c);
        ...
        {
            return math.mul(q, math.select(quaternion.identity.value, mask * fp.halfsqrt2, c));
        }
        ...
        static void sortSingularValues(ref float3x3 b, ref quaternion v)
        {
            ...
            v = condNegSwapQuat(c, v, math.float4(fp._0, fp._0, fp._1, fp._1));
            condSwap(c, ref l0, ref l1);

            c = l0 < l2;
            condNegSwap(c, ref b.c0, ref b.c2);
            v = condNegSwapQuat(c, v, math.float4(fp._0, -fp._1, fp._0, fp._1));
            condSwap(c, ref l0, ref l2);

            c = l1 < l2;
            condNegSwap(c, ref b.c1, ref b.c2);
            v = condNegSwapQuat(c, v, math.float4(fp._1, fp._0, fp._0, fp._1));
        }

        static readonly fp c8 = fp._cos_pidiv8; // cos(pi/8)
        static readonly fp s8 = fp._sin_pidiv8; // sin(pi/8)
        static readonly fp g = fp._3 + fp._2*fp.sqrt2; // 3 + 2 * sqrt(2)

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static quaternion approxGivensQuat(float3 pq, float4 mask)
        {
            var ch = fp._2 * (pq.x - pq.y); // approx cos(a/2)
        ...
        static quaternion qrGivensQuat(float2 pq, float4 mask)
        {
            ...
            var sh = math.select(fp._0, pq.y, l > k_EpsilonNormalSqrt);
            var ch = math.abs(pq.x) + math.max(l, k_EpsilonNormalSqrt);
            condSwap(pq.x < fp._0, ref sh, ref ch);
        ...
        {
            var u = qrGivensQuat(math.float2(b.c0.x, b.c0.y), math.float4(fp._0, fp._0, fp._1, fp._1));
            var qmt = math.float3x3(math.conjugate(u));
            r = math.mul(qmt, b);

            var q = qrGivensQuat(math.float2(r.c0.x, r.c0.z), math.float4(fp._0, -fp._1, fp._0, fp._1));
            u = math.mul(u, q);
            qmt = math.float3x3(math.conjugate(q));
            r = math.mul(qmt, r);

            q = qrGivensQuat(math.float2(r.c1.y, r.c1.z), math.float4(fp._1, fp._0, fp._0, fp._1));
            u = math.mul(u, q);
            qmt = math.float3x3(math.conjugate(q));
            r = math.mul(qmt, r);
        ...
        static quaternion jacobiIteration(ref float3x3 s, int iterations = 5)
        {
            ...
            {
                q = approxGivensQuat(math.float3(s.c0.x, s.c1.y, s.c0.y), math.float4(fp._0, fp._0, fp._1, fp._1));
                v = math.mul(v, q);
                qm = math.float3x3(q);
                s = math.mul(math.mul(math.transpose(qm), s), qm);

                q = approxGivensQuat(math.float3(s.c1.y, s.c2.z, s.c1.z), math.float4(fp._1, fp._0, fp._0, fp._1));
                v = math.mul(v, q);
                qm = math.float3x3(q);
                s = math.mul(math.mul(math.transpose(qm), s), qm);

                q = approxGivensQuat(math.float3(s.c2.z, s.c0.x, s.c2.x), math.float4(fp._0, fp._1, fp._0, fp._1));
                v = math.mul(v, q);
                qm = math.float3x3(q);
                s = math.mul(math.mul(math.transpose(qm), s), qm);
            }
        ...
        public static readonly fp k_EpsilonDeterminant = fp.epsilon_e6f;
        public static readonly fp k_EpsilonRCP = fp.epsilon_e9f;
        public static readonly fp k_EpsilonNormalSqrt = fp.epsilonroot;
        public static readonly fp k_EpsilonNormal = fp.epsilon;

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static float3 rcpsafe(float3 x, fp epsilon) =>
            math.select(math.rcp(x), float3.zero, math.abs(x) < epsilon);
        static float3 rcpsafe(float3 x) =>
