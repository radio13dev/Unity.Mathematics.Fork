//
// float3  psrdnoise(float2 pos, float2 per, fp rot)
// float3  psrdnoise(float2 pos, float2 per)
// fp psrnoise(float2 pos, float2 per, fp rot)
// fp psrnoise(float2 pos, float2 per)
// float3  srdnoise(float2 pos, fp rot)
// float3  srdnoise(float2 pos)
// fp srnoise(float2 pos, fp rot)
// fp srnoise(float2 pos)
//
...
//
// float3 {p}s{r}dnoise(float2 pos {, float2 per} {, fp rot})
// "pos" is the input (x,y) coordinate
// "per" is the x and y period, where per.x is a positive integer
//    and per.y is a positive even integer
// "rot" is the angle to rotate the gradients (any fp value,
...
//
// fp {p}s{r}noise(float2 pos {, float2 per} {, fp rot})
// "pos" is the input (x,y) coordinate
// "per" is the x and y period, where per.x is a positive integer
//    and per.y is a positive even integer
// "rot" is the angle to rotate the gradients (any fp value,
...
namespace Unity.Mathematics.Fixed
{
    public static partial class noise
    {
        ...
        public static float3 psrdnoise(float2 pos, float2 per, fp rot)
        {
            // Hack: offset y slightly to hide some rare artifacts
            pos.y += fp._0_01;
            // Skew to hexagonal grid
            float2 uv = float2(pos.x + pos.y * fp._0_50, pos.y);

            float2 i0 = floor(uv);
            float2 f0 = frac(uv);
            // Traversal order
            float2 i1 = (f0.x > f0.y) ? float2(fp._1, fp._0) : float2(fp._0, fp._1);

            // Unskewed grid points in (x,y) space
            float2 p0 = float2(i0.x - i0.y * fp._0_50, i0.y);
            float2 p1 = float2(p0.x + i1.x - i1.y * fp._0_50, p0.y + i1.y);
            float2 p2 = float2(p0.x + fp._0_50, p0.y + fp._1);
            ...
            float3 t = fp._0_8 - float3(dot(d0, d0), dot(d1, d1), dot(d2, d2));

            // Partial derivatives for analytical gradient computation
            float3 dtdx = -fp._2 * float3(d0.x, d1.x, d2.x);
            float3 dtdy = -fp._2 * float3(d0.y, d1.y, d2.y);
            ...
            float2 dt0 = float2(dtdx.x, dtdy.x) * fp._4 * t3.x;
            float2 dn0 = t4.x * g0 + dt0 * w.x;
            float2 dt1 = float2(dtdx.y, dtdy.y) * fp._4 * t3.y;
            float2 dn1 = t4.y * g1 + dt1 * w.y;
            float2 dt2 = float2(dtdx.z, dtdy.z) * fp._4 * t3.z;
            float2 dn2 = t4.z * g2 + dt2 * w.z;

            return fp._11 * float3(n, dn0 + dn1 + dn2);
        }
        ...
        {
            return psrdnoise(pos, per, fp._0);
        }
        ...
        public static fp psrnoise(float2 pos, float2 per, fp rot)
        {
        ...
        /// <param name="per">The x and y period, where per.x is a positive integer and per.y is a positive even integer.</param>
        /// <returns>Noise value.</returns>
        public static fp psrnoise(float2 pos, float2 per)
        {
            return psrnoise(pos, per, fp._0);
        }
        ...
        /// <param name="rot">Angle to rotate the gradients.</param>
        /// <returns>The first component of the 3-element return vector is the noise value, and the second and third components are the x and y partial derivatives.</returns>
        public static float3 srdnoise(float2 pos, fp rot)
        {
            // Offset y slightly to hide some rare artifacts
            pos.y += fp._0_001;
            // Skew to hexagonal grid
        ...
        /// <param name="rot">Angle to rotate the gradients.</param>
        /// <returns>Noise value.</returns>
        public static fp srnoise(float2 pos, fp rot)
        {
            // Offset y slightly to hide some rare artifacts
            pos.y += fp._0_001;
            // Skew to hexagonal grid
        ...
        /// <param name="pos">Input (x,y) coordinate.</param>
        /// <returns>Noise value.</returns>
        public static fp srnoise(float2 pos)
        {
            return srnoise(pos, fp._0);
        }
    }
}
