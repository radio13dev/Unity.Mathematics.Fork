...
namespace Unity.Mathematics.Fixed
{
    ...
    {
        static readonly fp Ko = fp._0_50-Kdiv2; // 1/2-K/2
        static readonly fp Kpow2 = fp._1/((fp)49); // 1/(7*7)
        static readonly fp Kz = fp._1/((fp)6); // 1/6
        static readonly fp Kzo = fp._0_50-fp._2/((fp)6); // 1/2-1/6*2
        ...
        public static float2 cellular2x2x2(float3 P)
        {
            ...
            float3 Pf = frac(P);
            float4 Pfx = Pf.x + float4(fp._0, -fp._1, fp._0, -fp._1);
            float4 Pfy = Pf.y + float4(fp._0, fp._0, -fp._1, -fp._1);
            float4 p = permute(Pi.x + float4(fp._0, fp._1, fp._0, fp._1));
            p = permute(p + Pi.y + float4(fp._0, fp._0, fp._1, fp._1));
            float4 p1 = permute(p + Pi.z); // z+0
            float4 p2 = permute(p + Pi.z + float4(fp._1,fp._1,fp._1,fp._1)); // z+1
            float4 ox1 = frac(p1 * K) - Ko;
            float4 oy1 = mod7(floor(p1 * K)) * K - Ko;
            float4 oz1 = floor(p1 * jitter_0_80) * Kz - Kzo; // p1 < 289 guaranteed
            float4 ox2 = frac(p2 * K) - Ko;
            float4 oy2 = mod7(floor(p2 * K)) * K - Ko;
            float4 oz2 = floor(p2 * Kpow2) * Kz - Kzo;
            ...
            float4 dz2 = Pf.z - fp._1 + jitter * oz2;
