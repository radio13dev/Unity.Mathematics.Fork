...
namespace Unity.Mathematics.Fixed
{
    public static partial class noise
    {
        ...
        public static float2 cellular(float3 P)
        {
            ...
            float3 Pf = frac(P) - fp._0_50;

            float3 Pfx = Pf.x + float3(fp._1, fp._0, -fp._1);
            float3 Pfy = Pf.y + float3(fp._1, fp._0, -fp._1);
            float3 Pfz = Pf.z + float3(fp._1, fp._0, -fp._1);

            float3 p = permute(Pi.x + float3(-fp._1, fp._0, fp._1));
            float3 p1 = permute(p + Pi.y - fp._1);
            float3 p2 = permute(p + Pi.y);
            float3 p3 = permute(p + Pi.y + fp._1);

            float3 p11 = permute(p1 + Pi.z - fp._1);
            float3 p12 = permute(p1 + Pi.z);
            float3 p13 = permute(p1 + Pi.z + fp._1);

            float3 p21 = permute(p2 + Pi.z - fp._1);
            float3 p22 = permute(p2 + Pi.z);
            float3 p23 = permute(p2 + Pi.z + fp._1);

            float3 p31 = permute(p3 + Pi.z - fp._1);
            float3 p32 = permute(p3 + Pi.z);
            float3 p33 = permute(p3 + Pi.z + fp._1);

            float3 ox11 = frac(p11 * K) - Ko;
            float3 oy11 = mod7(floor(p11 * K)) * K - Ko;
            float3 oz11 = floor(p11 * Kpow2) * Kz - Kzo; // p11 < 289 guaranteed

            float3 ox12 = frac(p12 * K) - Ko;
            float3 oy12 = mod7(floor(p12 * K)) * K - Ko;
            float3 oz12 = floor(p12 * Kpow2) * Kz - Kzo;

            float3 ox13 = frac(p13 * K) - Ko;
            float3 oy13 = mod7(floor(p13 * K)) * K - Ko;
            float3 oz13 = floor(p13 * Kpow2) * Kz - Kzo;

            float3 ox21 = frac(p21 * K) - Ko;
            float3 oy21 = mod7(floor(p21 * K)) * K - Ko;
            float3 oz21 = floor(p21 * Kpow2) * Kz - Kzo;

            float3 ox22 = frac(p22 * K) - Ko;
            float3 oy22 = mod7(floor(p22 * K)) * K - Ko;
            float3 oz22 = floor(p22 * Kpow2) * Kz - Kzo;

            float3 ox23 = frac(p23 * K) - Ko;
            float3 oy23 = mod7(floor(p23 * K)) * K - Ko;
            float3 oz23 = floor(p23 * Kpow2) * Kz - Kzo;

            float3 ox31 = frac(p31 * K) - Ko;
            float3 oy31 = mod7(floor(p31 * K)) * K - Ko;
            float3 oz31 = floor(p31 * Kpow2) * Kz - Kzo;

            float3 ox32 = frac(p32 * K) - Ko;
            float3 oy32 = mod7(floor(p32 * K)) * K - Ko;
            float3 oz32 = floor(p32 * Kpow2) * Kz - Kzo;

            float3 ox33 = frac(p33 * K) - Ko;
            float3 oy33 = mod7(floor(p33 * K)) * K - Ko;
            float3 oz33 = floor(p33 * Kpow2) * Kz - Kzo;
